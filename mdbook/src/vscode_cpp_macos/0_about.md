...
## 简介
注意：此教程使用`Intel Mac`、`macOS Big Sur`录制，对于`M1 Mac`一些章节可能并不使用（如第7课 用`VS Code`调试`Cpp`代码）。由于苹果逐渐转向自研芯片，此教程会在约4年后（2025年）（及这4年间）部分过时，还请注意。这期间讲稿会持续更新，以修正或补充视频中的一些错误或疏忽；之后也会考虑重新录制视频教程，但由于我目前手上没有`M1 Mac`，这件事情可能会长期搁置，还请谅解。
